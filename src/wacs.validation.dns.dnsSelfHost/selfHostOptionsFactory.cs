...
{
    internal class SelfDNSOptionsFactory : ValidationPluginOptionsFactory<SelfDNS, SelfDNSOptions>
    {
        ...
        public override async Task<SelfDNSOptions> Aquire(Target target, IInputService input, RunLevel runLevel)
    {
            ...
            using (DnsServerAcme server = new DnsServerAcme(_log))
            {
                ...
                //        var lookupClients = await _dnsClient.GetAuthority(serverIP.ToString(), 0);
                //        var lookupclient = lookupClients.Nameservers.First;
                //        var TXTResponse = lookupclient.fi(identifiers.First(),0);
        ...
        public override Task<SelfDNSOptions> Default(Target target)
        {
            return Task.FromResult(new SelfDNSOptions());
        }
